...
public sealed class ProcurementPhase4Tests
{
    ...
    [TestMethod]
    public async Task CommunicationCanHaveNoPriceOrSeveralPricesAndAgreedIsNotAcquired()
    {
        ...
        await fixture.Workspace.AddNegotiationAsync(fixture.Manager,
            new(taken.CaseId, current.Item.CaseVersion, null, null, null, "Звонок", "Собственник", "Не дозвонились",
                "", "Попытка связи", "Позвонить позже", DateTimeOffset.UtcNow.AddDays(1), DateTimeOffset.UtcNow.AddMinutes(-2)),
            "no-price", CancellationToken.None);
        current = await fixture.Workspace.ReadCardAsync(fixture.Manager, taken.CaseId, CancellationToken.None);
        await fixture.Workspace.AddNegotiationAsync(fixture.Manager,
            new(taken.CaseId, current.Item.CaseVersion, 16_600_000m, 16_000_000m, 16_300_000m, "Встреча", "Собственник",
                "Договорились по цене", "Без дополнительных условий", "Один реальный разговор", "Получить документы", null,
                DateTimeOffset.UtcNow.AddMinutes(-1)), "multi-price", CancellationToken.None);

        CaseCard card = await fixture.Workspace.ReadCardAsync(fixture.Manager, taken.CaseId, CancellationToken.None);
        Assert.AreEqual(2, card.Negotiations.Count);
        Assert.IsNull(card.Negotiations.Single(item => item.Outcome == "Не дозвонились").SellerPrice);
        NegotiationView priced = card.Negotiations.Single(item => item.Outcome == "Договорились по цене");
        Assert.AreEqual(16_600_000m, priced.SellerPrice); Assert.AreEqual(16_000_000m, priced.BuyerOffer); Assert.AreEqual(16_300_000m, priced.AgreedPrice);
    ...
    public async Task ReturnReworkResubmitPreservesTaskHistoryAndEnablesStructuredChecks()
    {
        ...
        await fixture.Workspace.SaveCheckAsync(fixture.Manager,
            new(taken.CaseId, null, card.Item.CaseVersion, null, CaseCheckLevel.Quick, "Сверить кадастровую карту",
                CaseCheckStatus.Passed, fixture.ManagerEmployeeId, true, null, null, "Границы совпадают", false), "quick", CancellationToken.None);
        ...
        await Assert.ThrowsExactlyAsync<AccessDeniedException>(() => fixture.Workspace.SaveCheckAsync(fixture.Manager,
            new(taken.CaseId, quick.Id, card.Item.CaseVersion, quick.Version, quick.Level, quick.Title,
                CaseCheckStatus.Blocked, fixture.ManagerEmployeeId, true, null, null, "Найден риск", true), "blocker-denied", CancellationToken.None));
        ...
        await fixture.Workspace.SaveCheckAsync(fixture.Head,
            new(taken.CaseId, quick.Id, card.Item.CaseVersion, quick.Version, quick.Level, quick.Title,
                CaseCheckStatus.Blocked, fixture.ManagerEmployeeId, true, null, null, "Проезд требует юридического подтверждения", true),
        ...
        await fixture.Workspace.SaveCheckAsync(fixture.Manager,
            new(taken.CaseId, null, card.Item.CaseVersion, null, CaseCheckLevel.Deep, "Юридическая проверка права",
                CaseCheckStatus.InProgress, fixture.ManagerEmployeeId, true, DateTimeOffset.UtcNow.AddDays(3), 20_000m,
    ...
    public async Task AttachmentsFollowOwningCaseScopeAndHideStorageKey()
    {
        ...
        CaseCard card = await fixture.Workspace.ReadCardAsync(fixture.Manager, taken.CaseId, CancellationToken.None);
        await fixture.Workspace.AddNegotiationAsync(fixture.Manager,
            new(taken.CaseId, card.Item.CaseVersion, 2_000_000m, null, null, "Сообщение", "Продавец",
                "Получено предложение", "Без условий", "", "Ответить", null, DateTimeOffset.UtcNow.AddMinutes(-1)), "neg", CancellationToken.None);
        card = await fixture.Workspace.ReadCardAsync(fixture.Manager, taken.CaseId, CancellationToken.None);
        ...
        await Assert.ThrowsExactlyAsync<AccessDeniedException>(() => fixture.Workspace.AddNegotiationAsync(fixture.SecondManager,
            new(taken.CaseId, card.Item.CaseVersion, null, 1_900_000m, null, "Звонок", "Продавец",
                "Нет доступа", "", "", "", null, DateTimeOffset.UtcNow.AddMinutes(-1)), "scope-neg", CancellationToken.None));
        await Assert.ThrowsExactlyAsync<AccessDeniedException>(() => fixture.Workspace.SaveCheckAsync(fixture.SecondManager,
            new(taken.CaseId, null, card.Item.CaseVersion, null, CaseCheckLevel.Quick, "Чужая проверка",
                CaseCheckStatus.Planned, null, false, null, null, "", false), "scope-check", CancellationToken.None));
        ...
        Guid fileId = await fixture.Workspace.AddAttachmentAsync(fixture.Manager,
            new(taken.CaseId, CaseAttachmentOwner.Case, null, CaseAttachmentKind.Document, "Заключение", "Понятный комментарий", "conclusion.txt", "text/plain", bytes, null),
            "file", CancellationToken.None);
        Guid linkId = await fixture.Workspace.AddAttachmentAsync(fixture.Manager,
            new(taken.CaseId, CaseAttachmentOwner.Negotiation, negotiationId, CaseAttachmentKind.Link, "Переписка", "Контекст разговора", "", "", null, "https://example.test/dialog"),
