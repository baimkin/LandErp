...
B3-01 и B3-02 вместе, исправить найденные мелкие дефекты и встроить функции в
актуальный дизайн карточки, не возвращая старую разметку.
...
storage recovery и точечные проверки. Существующий дизайн карточки и разрешение
редакторам досье добавлять заметки сохранить.

Required reading: `P0_CATALOG_PROCUREMENT_BOUNDARY.md`,
`STAGE-1_DATA_CONVENTIONS.md`, относящиеся положения FP-004,
`GATE-RELEASE-B2-01.md`–`GATE-RELEASE-B2-04.md`,
`GATE-RELEASE-B3-01.md`, `GATE-RELEASE-B3-02.md` и их отчёты.

## Предыдущая работа — интеграция карточки и B1-01–B1-04

**Прямой запрос владельца от 18 сентября 2026 года:** объединить уже выполненные
параллельные изменения в один проверяемый результат, прогнать проверки и запускать
для ручной приёмки только объединённую версию.

**Интеграционная ветка:** `codex/integrate-property-card-b1`.
**База дизайна:** `e1ad4c708ebf2b2f413efe1c1f224fb0bb509e21`.
**Пакет B1-01–B1-04:** `92e952d0ecce6b5235b4c5d13dc0040dc2c236fc`.

Scope: сохранить актуальный дизайн карточки/осмотра, встроить уникальные гарантии
B1-01–B1-04, разрешить только реальные конфликты, выполнить Release build,
целевые тесты пакета и карточки, затем запустить локальный Server на существующей
БД. Изменение production и переписывание опубликованных веток не входят.

Код B1-04 на `59181519a89608e84d0c360ef8dae8c7708eefcc` прошёл удалённые
Release build и PostgreSQL targeted suite 40/40. Объединённая версия с актуальным
дизайном проверяется и запускается отдельно для ручной приёмки владельца.

Required reading: `GATE-PROPERTY-CARD-CONCEPT.md`, `RELEASE_PACKAGE_01_PLAN.md`,
`GATE-RELEASE-B1-01.md`, `GATE-RELEASE-B1-02.md`, `GATE-RELEASE-B1-03.md`,
`GATE-RELEASE-B1-04.md`, их
отчёты и только затрагиваемый код/тесты.

## Предыдущая работа — карточка по утверждённому концепту

> **Реализовано:** [GATE-PROPERTY-CARD-CONCEPT.md](GATE-PROPERTY-CARD-CONCEPT.md), ветка `codex/property-card-concept`. Перенос утверждённого автономного концепта в реальную карточку и осмотр.
