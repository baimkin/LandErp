...
try {
    ...
    & $DotnetPath test (Join-Path $storageRoot 'tests/LandErp.Foundation.Tests') -c Release --no-build --filter 'TestCategory=LiveStorage' --logger 'console;verbosity=minimal'
