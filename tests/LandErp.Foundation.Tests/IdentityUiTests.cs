...
public sealed class IdentityUiTests
{
    ...
    public async Task RealCookieMfaAdminPersistenceCsrfAndResponsiveStates()
    {
        ...
        try
        {
            ...
            await employeePage.GotoAsync(origin + "/collectors");
            await employeePage.GetByRole(AriaRole.Heading, new() { Name = "Нет доступа", Exact = true }).WaitForAsync();
            ...
            await page.GetByLabel("Название группы", new() { Exact = true }).FillAsync("Автотесты");
            await page.GetByRole(AriaRole.Button, new() { Name = "Создать группу", Exact = true }).ClickAsync();
            await page.Locator(".list-row", new() { HasText = "Автотесты" }).WaitForAsync();
            ...
            Assert.IsTrue(await page.EvaluateAsync<bool>("document.documentElement.scrollWidth <= window.innerWidth"));
            await page.ScreenshotAsync(new() { Path = Path.Combine(collectorImages, "tablet.png"), FullPage = true });
        }
