...
public static class ProcurementLabels
{
    ...
    public static string Stage(string stage) => stage switch { "new" => "Новое", "analysis" => "В работе", "clarify" => "Уточнить", "monitor" => "Наблюдать", "rejected" => "Отклонён", "pending_head" => "У руководителя", "returned" => "Возвращён", "approved" => "Одобрен", "negotiation" => "Переговоры и проверки", _ => "Неизвестный этап" };
    ...
    public static string Negotiation(NegotiationPriceType type) => type switch { NegotiationPriceType.Ask => "Стартовая цена", NegotiationPriceType.SellerOffer => "Цена продавца", NegotiationPriceType.BuyerOffer => "Наше предложение", _ => "Согласованная цена" };
