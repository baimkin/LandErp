...
public static class ProcurementLabels
{
    ...
    public static string DocumentStatus(CaseDocumentStatus status) => status switch { CaseDocumentStatus.Missing => "Нет", CaseDocumentStatus.Requested => "Запрошен", CaseDocumentStatus.Received => "Получен", CaseDocumentStatus.Verified => "Проверен", _ => status.ToString() };
    ...
    public static string FileStatus(StoredFileStatus status) => status switch { StoredFileStatus.PendingUpload => "Загрузка не подтверждена", StoredFileStatus.Available => "Доступно", StoredFileStatus.UploadFailed => "Ошибка загрузки — можно повторить", StoredFileStatus.Quarantined => "На проверке", StoredFileStatus.Rejected => "Отклонено", _ => "Удалено" };
