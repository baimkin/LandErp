...
## Текущий Gate — B1-01, первый пакет доведения до релиза
...
B1-02, B1-03 и B1-04 **не начаты**. Следующую задачу активировать только по отдельному запросу владельца. Main, merge/rebase/force-push, сброс/изменение рабочей БД, запуск Server/Worker/Parser и релиз не входят в текущее разрешение.
