...
$applicationSource = Join-Path $backup 'application'
if ($null -eq $manifest.ApplicationFiles -or -not (Test-Path -LiteralPath $applicationSource)) { throw 'Application backup manifest/files are missing.' }
$applicationPrefix = [IO.Path]::GetFullPath($applicationSource).TrimEnd('\') + '\'
foreach ($entry in @($manifest.ApplicationFiles)) {
    $relative = [string]$entry.Path
    if ([string]::IsNullOrWhiteSpace($relative) -or [IO.Path]::IsPathRooted($relative)) { throw 'Application backup manifest contains an invalid path.' }
    $file = [IO.Path]::GetFullPath((Join-Path $applicationSource $relative.Replace('/','\')))
    if (-not $file.StartsWith($applicationPrefix, [StringComparison]::OrdinalIgnoreCase) -or -not (Test-Path -LiteralPath $file -PathType Leaf)) { throw 'Application backup manifest points outside the backup or to a missing file.' }
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $file).Hash -ne [string]$entry.Sha256) { throw "Application backup hash mismatch: $relative" }
}
...
if (-not [string]::IsNullOrWhiteSpace($ApplicationFilesDestination)) {
    $destination = [IO.Path]::GetFullPath($ApplicationFilesDestination)
    if (Test-Path -LiteralPath $destination) { throw 'ApplicationFilesDestination must not already exist.' }
    New-Item -ItemType Directory -Path $destination | Out-Null
    & icacls.exe $destination /inheritance:r /grant:r '*S-1-5-18:(OI)(CI)(F)' '*S-1-5-32-544:(OI)(CI)(F)' | Out-Null
    if ($LASTEXITCODE -ne 0) { throw 'Could not restrict restored application files ACL.' }
    Copy-Item -Path (Join-Path $applicationSource '*') -Destination $destination -Recurse
    Write-Output "Application config/keys restored to isolated private path: $destination"
}
