...
### ListingData enrichment

V1 остаётся additive: старый Parser без новых полей по-прежнему валиден. Для объявления
могут дополнительно передаваться:

- `CadastralNumber` — typed text field; отсутствие не стирает ранее известное значение;
- `SourcePublishedAt` — UTC дата публикации/добавления на площадке. Она не заменяет
  `ObservedAt`, `FirstObservedAt` или время получения Server;
- `Latitude` + `Longitude` — только парой, WGS84 / EPSG:4326;
- `DeclaredLandTypes[]` — нормализованные типы, структурированно заявленные площадкой.
  Это не юридически подтверждённый ВРИ или категория земли;
- `Contacts[]` — публичные контакты объявления: Phone, Email, Telegram, WhatsApp,
  Website или Other. Parser передаёт наблюдаемое значение и optional display/primary;
  Server сам нормализует identity и выполняет дедупликацию.

Server отдельно вычисляет source-text классификацию участка из Title/Description. В Catalog
сохраняются оба слоя: «заявлено источником» и «найдено в тексте». Их расхождение является
диагностическим бизнес-сигналом, а не выбором одной «истины».

Контакты хранятся отдельно от Listing. Повтор того же нормализованного контакта обновляет
границы FirstObservedAt/LastObservedAt. Контакт не удаляется только потому, что следующая
страница/снимок его не показали. Этот V1 foundation не объединяет продавцов между разными
объявлениями и не является CRM-моделью.
